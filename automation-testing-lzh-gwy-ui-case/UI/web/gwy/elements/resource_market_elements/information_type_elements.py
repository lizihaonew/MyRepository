...
'''
作者： 李子浩
内容：顾问云资源市场类别管理中的资讯二级类别管理相关元素集合
'''
...
class FirstTypeListElement(ElementCollection):
    ...
    '''
    def __init__(self):
        self.information = AsyncElement('tr:nth-child(2) > td:nth-child(2) > a')    # 资讯
...
class TypeListElements(ElementCollection):
    ...
    def __init__(self):
        ...
        self.first_row_name = AsyncElement('div.ibox-content > div > table > tbody > tr:nth-child(1) > td:nth-child(1)')    # 列表中第一行的二级类别名称
        self.first_row_update = AsyncElement('tr:nth-child(1) > td:nth-child(3) > a:nth-child(1)')  # 列表中第一行的修改按钮
        self.first_row_delete = AsyncElement('tr:nth-child(1) > td:nth-child(3) > a:nth-child(2)')  # 列表中第一行的删除按钮
