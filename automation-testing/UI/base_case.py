...
class BaseUITestCase(unittest.TestCase):
    ...
    def _get_user_data_from_login_api(cls):
        response = requests.post('{0}{1}'.format(config.TEST_SERVER_ADDRESS[cls.driver_type][cls.project_name], config.LOGIN_API_ENDPOINT),
                                 data=config.DATA, headers=config.HEADER)
    ...
    def _dummy_login(cls):
        ...
        """
        cls.browser.execute_script('localStorage.setItem("userData", arguments[0]);',
