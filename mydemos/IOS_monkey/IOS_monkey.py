...
import configparser
import os
from get_ios_log import get_log_file, analyse_log_file, get_device_info
...
def run_loop(times, start_time):
    ...
    if times <5:
        try:
            ...
            while True:
                pack_name = ios_monkey.is_element_exist("name", "当当测试包")
                pack_label = ios_monkey.is_element_exist("label", "当当测试包")
                if not pack_name and not pack_label:
                    ios_monkey.close_driver()
                    print("未获取到name=当当测试包的元素")
                    run_loop(times, start_time)
                # login_name = ios_monkey.is_element_exist("name", "账号密码登录")
                # if login_name:
                #     phone_input = ios_monkey.find_element("value", "请输入手机号/邮箱/昵称")
                #     phone_input.clear()
                #     phone_input.send_keys('11111')
                #     passwd_input = ios_monkey.find_element("value", "请输入密码")
                #     passwd_input.clear()
                #     passwd_input.send_keys('22222222')
                #     types = ios_monkey.find_elements("type", "XCUIElementTypeStaticText")
                #     print(types)
                #     ios_monkey.find_element("name", "登录").click()
                #     break
        ...
        raise Exception('程序运行失败，重试5次后运行结束...')
...
def main():
    ...
    times = 0
    run_loop(times, start_time)
    res_dict = analyse_log_file(log_path)
    print(res_dict)
